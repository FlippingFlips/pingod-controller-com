...
Class cvpmTrough
	...
	Private Sub UpdateDebugBox   ' Requires a textbox named DebugBox
		...
		If mEntrySw > 0 Then
			mSwcopy = mEntrySw
			str = str & Controller.GetSwitch(mSwcopy)
		Else
			str = str & "n/a"
		End If
		str = str & ")" & vbNewLine
		...
		For ii = UBound(mSlot) To 0 Step -1
			If ii Mod 2 = 0 Then
				If mSw(ii\2) Then
					mSwcopy = mSw(ii\2)
					If Controller.GetSwitch(mSwcopy) Then
						str = str & "1"
					Else
						str = str & "0"
					End If
...
Class cvpmBallStack
	...
	Public Sub AddBall(aKicker)
		...
		If mEntrySw Then
			mSwcopy = mEntrySw
			Controller.Switch mSwcopy, 1: mBallIn = mBallIn + 1
		Else
...
Public Sub vpmInit(aTable)
	...
	If vpmVPVer >= 60 Then
	  On Error Resume Next
		If Not IsObject(GetRef(aTable.name & "_Paused")) Or Err Then Err.Clear : vpmBuildEvent aTable, "Paused", "Controller.Pause 1"
		If Not IsObject(GetRef(aTable.name & "_UnPaused")) Or Err Then Err.Clear : vpmBuildEvent aTable, "UnPaused", "Controller.Pause 0"
		If Not IsObject(GetRef(aTable.name & "_Exit")) Or Err Then Err.Clear : vpmBuildEvent aTable, "Exit", "Controller.Pause 0:Controller.Stop"
	End If
	if UseModSol Then
	...
	If Not IsEmpty(ChgLed) Then		
		On Error Resume Next			
			For ii = 0 To UBound(ChgLed)
				...
				if color = 0 Then state = 0 : color =  PdbOffColor  : Else state = 1 : End If 'if color is 0 = black, then disable the light and set off color
				If IsArray(Lights(idx)) Then
					For Each tmp In Lights(idx) : tmp.Color = color : tmp.FullColor = color : tmp.State = state : Next
				Else
					Lights(idx).Color = color : Lights(idx).FullColor = color : Lights(idx).State = state
				End If
			Next
			For Each tmp In vpmMultiLights
				For ii = 1 To UBound(tmp) : tmp(ii).Color = tmp(0).Color : tmp(ii).FullColor = tmp(0).FullColor : tmp(ii).State = tmp(0).State : Next
			Next
			PDLedCallback
		On Error Goto 0
	End If
...
Sub vpmCreateEvents(aHitObjs)
	...
	For Each obj In aHitObjs
		Select Case TypeName(obj)
			Case "Trigger"
				vpmBuildEvent obj, "Hit", "Controller.Switch " & Obj.TimerInterval & ", 1"
				vpmBuildEvent obj, "UnHit", "Controller.Switch " & Obj.TimerInterval & ", 0"
			Case "Wall"
